...
\begin{tikzpicture}[
    ...
    initial text= % não é necessário texto aqui
  ]
  ...
  \path (qsb) edge[loop left] node {Y / Y D} (qsb)
              ...
              edge[bend left=45] node[above] {$\blank$ / $\blank$ E} (acc);
  ...
  at (1.25, -12.5) {
    ...
    Qualquer transição não desenhada para um par\\ (Estado, Símbolo) implica parada e rejeição.
  };
